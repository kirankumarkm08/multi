...
export default function EditRegistrationPage() {
  ...
  }, [token, isInitialized, pageId]);

  const loadPage = async (id: string) => {
    try {
      setLoading(true);
      const result = await apiFetch(`/tenant/pages/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });

      const data = result.data ?? result; // fallback if no .data

      // --- Parse form_config as array of fields ---
      let formConfigParsed: FormField[] = [];
      if (typeof data.form_config === "string") {
        try {
          const parsed = JSON.parse(data.form_config);
          formConfigParsed = Array.isArray(parsed.fields) ? parsed.fields : [];
        } catch {
          formConfigParsed = [];
        }
      } else if (Array.isArray(data.form_config)) {
        formConfigParsed = data.form_config;
      } else if (Array.isArray(data.form_config?.fields)) {
        formConfigParsed = data.form_config.fields;
      }

      // --- Parse settings JSON ---
      let settingsParsed: PageSettings = {
      ...
      if (typeof data.settings === "string") {
        try {
          ...
          settingsParsed = {
            submitButtonText: parsed?.submitButtonText || "Register Now",
            successMessage: parsed?.successMessage || "Thank you for registering!",
            redirectUrl: parsed?.redirectUrl || "",
            show_in_nav: parsed?.show_in_nav ?? false,
          };
        } catch {
          // keep defaults
        }
      } else if (typeof data.settings === "object" && data.settings) {
        settingsParsed = {
          submitButtonText: data.settings.submitButtonText || "Register Now",
          successMessage: data.settings.successMessage || "Thank you for registering!",
          redirectUrl: data.settings.redirectUrl || "",
          show_in_nav: data.settings.show_in_nav ?? false,
        };
      }

      setPage({
        ...data,
        form_config: formConfigParsed,
            ...
            <Card className="dark:bg-gray-800 dark:border-gray-700">
              ...
              <CardContent>
                <div className="space-y-4">
                  {page.form_config && page.form_config.length > 0 ? (
                    page.form_config
