...
export interface TicketCreateData {
  ...
  ticket_logo?: File | null; // Image file for upload
}
...
export const ticketService = {
  ...
  async createTicket(data: TicketCreateData | FormData): Promise<Ticket> {
    try {
      const token = typeof window !== "undefined" ? localStorage.getItem("access_token") : null;
  ...
  async getEventEditions(): Promise<EventEdition[]> {
    try {
      const token = typeof window !== "undefined" ? localStorage.getItem("access_token") : null;
      const response = await tenantApi.getEventsForTickets(token || undefined);
